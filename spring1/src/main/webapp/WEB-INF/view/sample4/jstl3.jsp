...
<body>
	<div id="app">
		<!-- table의 semantic tag가 뭐지? 의미가 있는것 -->
		<table class="table table=hover">
			...
			<tbody>
				<c:forEach items="${list }" var="board">
					<tr>
						<td><button class="btn">${board.bno }</button></td>
						<td>${board.title }</td>
						<td>${board.writer }</td>
			...
			<ul class="pagination">
				...
				<c:forEach begin="${pagination.start}" end="${pagination.end }"
					...
					<c:if test="${i==pagination.pageno }">
						<li class="page-item active"><a class="page-link" href="/sample4/jstl4?pageno=${i}">${i}</a>
						</li>
					</c:if>
					<!-- 
						controller는 작업을 통제
					 -->
					<c:if test="${i!=pagination.pageno }">
						<li class="page-item"><a class="page-link" href="/sample4/jstl4?pageno=${i}">${i}</a>
						</li>
					</c:if>
				</c:forEach>
